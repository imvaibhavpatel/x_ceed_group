...
class DistributorController extends GetxController {
  ...
  getCountry() async {
    try {
      final response = await http.get(
        Uri.parse(
            "${AppUrl.baseUrl}${AppUrl.countriesUrl}page=${countryPage.value}&country_name="),
      );
  ...
  getState(String countryID) async {
    try {
      final response = await http.get(Uri.parse(
          "${AppUrl.baseUrl}${AppUrl.stateURl}&country_id=$countryID&page=${statePage.value}&state_name="));
      if (response.statusCode == 200) {
  ...
  getCity(String stateID) async {
    try {
      final response = await http.get(Uri.parse(
          "${AppUrl.baseUrl}${AppUrl.cityUrl}&state_id=$stateID&page=${cityPage.value}&city_name="));
      if (response.statusCode == 200) {
